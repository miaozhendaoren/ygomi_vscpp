    ...
    if isempty(dbData{1})
        ...
        while dbDataIdx <= length(dbData)
            ...
            if(length(activeIdx) >= 6)
                Tr_fit
                [Tr_fit, activeIdx, matchIdx, closeIdx, minDist, meanDist] = icpMex(dbPaintArr,ndPaintArr,Tr_fit,2,'point_to_point',200);
                Tr_fit
