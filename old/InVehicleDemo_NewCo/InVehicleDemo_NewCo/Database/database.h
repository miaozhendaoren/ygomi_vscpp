...
namespace ns_database
{
    ...
    {
    private:
        static const int _dbFileNameMaxLen = 500;
        char _dbFileName[_dbFileNameMaxLen];
        FILE* _dbFid;
        fpos_t _fPosition;
        void*  _mPosition;
        uint8* _mEndPosition;
        ...
        std::list<std::list<std::vector<std::vector<tlvCommon_t>>>> _vectorList; // segment list / vector list / point list / point element
        std::list<std::list<uint8* >> _pointList;  // segment list / vector list / point array header
        ...
        void convPointElementToPoint3D(IN std::vector<tlvCommon_t>& pointElement, OUT point3D_t* point3dOut);
        ...
        void readTlvToPoint(IN  void** input,
                            IN  resource_e sourceFlag,
                            OUT std::vector<tlvCommon_t>& pointElement, 
    ...
    public:
        ...
        void addFurnitureTlv(IN uint8* tlvBuff, IN uint32 buffLen);

		void addFurniture(IN furAttributes_t* furnitureIn);

		void reduceFurnitureTlv(IN uint8* tlvBuff, IN uint32 buffLen);

        //void deleteSegmentById(IN uint32 segmentIdIn, OUT uint8* successFlag);

        //void deleteAllVectorsInSegById(IN uint32 segmentIdIn, OUT uint8* successFlag);
        ...
        void getAllFurnitures(OUT std::list<std::list<furAttributes_t>>& furnitureListOut);
        ...
        void getVectorsInSegTlv(IN uint32 segmentIdIn, 
                                IN resource_e sourceFlag, 
                                OUT void** output, 
                                OUT int32* length);
        ...
        void updateFurnitureByGps(IN  point3D_t*  gpsP, 
                                  IN  furAttributes_t* furnitureAttr);

		void reduceFurnitureByGps(IN  furAttributes_t* furnitureAttr);

		void resetFurniture();

        // methods for math
        bool checkGpsInRange(point3D_t* gpsA, point3D_t* gpsB, double distThresh);
        ...
        void calcGpsRoadSide(IN point3D_t* gpsPre, IN point3D_t* gpsCurr, IN uint8 side, IN float width, OUT point3D_t* gpsRight); // side: 1: right, 2: left

        void getLookAheadView(IN point3D_t* gpsCurr, IN float distanceIn, OUT point3D_t* gpsAhead);

		void getLookAheadFurnitures(IN point3D_t* gpsCurr, IN float distanceIn, OUT std::list<furAttributes_t>& furnitureAttrList, OUT std::list<point3D_t>& pointInRangeList);
    };
