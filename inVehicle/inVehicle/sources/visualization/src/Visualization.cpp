...
GLboolean OPENGL_3D_ENGINE::AddOneLineInfo(int number,lineTypeEnum_t type, point3DFloat_t* buffer)
{
	if((lineBuffer[lineBackBufIdx].number+1) > MAX_BUFFER_NUMBER_DRAW_LINE)
	{
		return GL_FALSE;
	}

	//copy the content of signInfo_t information
	memcpy((void*)lineBuffer[lineBackBufIdx].line[lineBuffer[lineBackBufIdx].number].position, (void*)buffer,number*sizeof(point3DFloat_t));

	//update the number
	lineBuffer[lineBackBufIdx].line[lineBuffer[lineBackBufIdx].number].number = number;
	lineBuffer[lineBackBufIdx].line[lineBuffer[lineBackBufIdx].number].type  = type;

	lineBuffer[lineBackBufIdx].number += 1;
	return GL_TRUE;
}

GLboolean OPENGL_3D_ENGINE::AddOneRoadLineInfo(int number,lineTypeEnum_t type, point3DFloat_t* buffer)
{
	if((roadLineBuffer[roadLineBackBufIdx].number+1) > MAX_BUFFER_NUMBER_DRAW_LINE)
	{
		return GL_FALSE;
	}

	//copy the content of signInfo_t information
	memcpy((void*)roadLineBuffer[roadLineBackBufIdx].line[roadLineBuffer[roadLineBackBufIdx].number].position, (void*)buffer,number*sizeof(point3DFloat_t));

	//update the number
	roadLineBuffer[roadLineBackBufIdx].line[roadLineBuffer[roadLineBackBufIdx].number].number = number;
	roadLineBuffer[roadLineBackBufIdx].line[roadLineBuffer[roadLineBackBufIdx].number].type  = type;

	roadLineBuffer[roadLineBackBufIdx].number += 1;
	return GL_TRUE;
}
...
void OPENGL_3D_ENGINE::DrawFrontBufferClient()
{
	...
	//draw road
	for(lineIdx = 0; lineIdx < (roadLineBuffer[roadLineFrontBufIdx].number/2); lineIdx++)
	{
		DrawRoadwithLine(&roadLineBuffer[roadLineFrontBufIdx].line[lineIdx*2],&roadLineBuffer[roadLineFrontBufIdx].line[lineIdx*2+1]);
	...
	for(lineIdx = 0; lineIdx < lineBuffer[lineFrontBufIdx].number; lineIdx++)
	{
		DrawLine(&lineBuffer[lineFrontBufIdx].line[lineIdx],ClientMode_3DEngine);
	}
...
void OPENGL_3D_ENGINE::DrawFrontBufferServer()
{
	...
	for(lineIdx = 0; lineIdx < lineBuffer[lineFrontBufIdx].number; lineIdx++)
	{
		DrawLine(&lineBuffer[lineFrontBufIdx].line[lineIdx],ServerMode_3DEngine);
	}
...
void OPENGL_3D_ENGINE::DrawClientMode()
{
	...
	if(eyeBufferLookahead[lookaheadBufIdx].number != 0)
	{
		...
		DrawCharView("Ahead View: 30M",121.0/255.0, 157.0/255.0, 248.0/255.0);
...
void OPENGL_3D_ENGINE::DrawLine(lineInfo_t *line,mode3DEngineEnum_t type)
{
	...
	if(lineTypeEnum_dash == line->type)
	{
		...
		if(ServerMode_3DEngine == type)
		{
			glLineStipple(1,0x0F0F);
			glLineWidth(1.0f);
		}else if(ClientMode_3DEngine == type)
		{
		...
		{
			if((line->position[index-1].x != line->position[index].x)&&
				(line->position[index-1].z != line->position[index].z))
			{
	...
	}else if(lineTypeEnum_solid == line->type)
	{
		...
		for(index = 0; index < line->number; index++)
		{
			glVertex3f(line->position[index].x, line->position[index].y+0.0001, line->position[index].z);
		}
		glEnd();
	}
...
void OPENGL_3D_ENGINE::DrawRoadSide(lineInfo_t* line1)
{
	...
	if(lineTypeEnum_roadside_line == line1->type)
	{
		...
		glBegin(GL_QUADS);
		for(index = 0; index < (line1->number-1); index++)
		{
...
void OPENGL_3D_ENGINE::DrawRoadwithLine(lineInfo_t* line1, lineInfo_t* line2)
{
	...
	int stripNum = (line1->number) > (line2->number) ?(line2->number):(line1->number);
	int index = 0;
    //GLint last_texture_ID;
	...
	if((line1->number) > (line2->number))
	{
		...
		for(index = (stripNum-1); index < (line1->number); index++)
		{
			glVertex3f(line1->position[index].x,line1->position[index].y,line1->position[index].z);
		}
	}
